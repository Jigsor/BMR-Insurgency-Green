...
addMissionEventHandler ["HandleDisconnect", {
	...
	if (typeOf _unit isEqualTo "HeadlessClient_F") then {
		{deleteVehicle _x} forEach (allUnits select {!(side _x isEqualTo Independent) && !(side _x isEqualTo civilian) && (_x isKindOf "Man")});
	};
	deleteVehicle _unit;
}];
...
if (Airfield_opt) then
{
	...
	switch (INS_op_faction) do {
		...
		case 9: {
			if (isClass(configFile >> "CfgVehicles" >> "LIB_US_P39")) then {
				_mod = true; _class = "LIB_US_P39";
	...
	0 spawn {
		if (!(getMarkerColor "AircraftMaintenance" isEqualTo "") || (markerAlpha "AircraftMaintenance" isEqualTo 1)) then {
			...
			_uavServiceTrig setTriggerActivation ["GUER","PRESENT",true];
...
diag_log "BMR Insurgency Internal Mission Version 1.504";
