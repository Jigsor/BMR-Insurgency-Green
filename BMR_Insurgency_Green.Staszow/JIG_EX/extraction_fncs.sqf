...
extraction_pos_fnc = {
	...
	if !(_newPos isEqualTo []) then {
		...
		[[_mkr],west] remoteExec ["Hide_Mkr_fnc", [0,-2] select isDedicated];
...
drop_off_pos_fnc = {
	...
	if !(_newPos isEqualTo []) then {
		...
		[[_tempPUmkr2],west] remoteExec ["Hide_Mkr_fnc", [0,-2] select isDedicated];
...
Ex_LZ_smoke_fnc = {
	...
	(localize "STR_BMR_heli_extraction_smoke") remoteExec ['JIG_EX_MPhint_fnc', INDEPENDENT];
