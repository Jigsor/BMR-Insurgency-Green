...
JIG_issue_reward = {
	...
	if (side _p == INS_Blu_side) then {
		...
		_p addrating 2000;
		_p addScore _pScore;
		[Resistance,"HQ"] sideChat "+20 points";
	...
	(_ammo == "C7_Remote_Ammo")) then {
		...
		if (!isNull _source && {isPlayer _source}) then {
			...
			[_rpos] spawn {
				params ["_pos"];
				private _uArr = [];
				{_uArr pushBack _x} forEach (_pos nearEntities ["CAManBase",200] select {((captiveNum _x isEqualTo 0) || (lifeState _x isEqualTo "HEALTHY") || (lifeState _x isEqualTo "INJURED")) && ((side _x isEqualTo Resistance) && (isPlayer _x))});
...
find_bombee_fnc = {
	...
	private _potentialTargets = playableUnits select {(vectorMagnitudeSqr velocity _x < 9) && ((getPosATL _x) select 2 < 3.1) && (side _x isEqualTo Resistance)};
