...
RandomAirCenterOp4 = {
	...
	_westbase1 = markerPos "Respawn_Guerrila";
...
// Seek N Destroy
			...
			if ((isNull _hunted) || (_vcl distance _hunted > _dis)) then {
				...
				_nrstWTgts = [];
				{_nrstWTgts pushBack _x} forEach ((position _vcl) nearEntities [["Air","CAManBase"], _dis] select {((captiveNum _x isEqualTo 0) || (lifeState _x isEqualTo "HEALTHY") || (lifeState _x isEqualTo "INJURED") || (_x getVariable ["ACE_isUnconscious", false])) && (side _x isEqualTo resistance)});
			...
			else
			{
				...
				_nrstWTgts = [];
				{_nrstWTgts pushBack _x} forEach ((position _vcl) nearEntities [["Air","CAManBase"], 2000] select {((captiveNum _x isEqualTo 0) || (lifeState _x isEqualTo "HEALTHY") || (lifeState _x isEqualTo "INJURED") || (_x getVariable ["ACE_isUnconscious", false])) && (side _x isEqualTo resistance)});
