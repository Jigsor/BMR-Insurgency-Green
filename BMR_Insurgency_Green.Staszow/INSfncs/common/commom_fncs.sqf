...
JIG_MPsideChatWest_fnc = {[Independent,"HQ"] SideChat (_this # 0)};
JIG_MPsideChatEast_fnc = {[West,"HQ"] SideChat (_this # 0)};
	...
	switch (true) do {
		case (_var isEqualTo "MHQ_1"): {
			if (INS_VA_type in [0,3]) then {
				...
				_veh addAction[("<t color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{["Open",true] call BIS_fnc_arsenal},nil,6,true,true,"","side _this != WEST",12];
			};
			if (INS_VA_type in [1,2]) then {
				_veh addAction[("<t color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{[_this] call JIG_VA},nil,6,true,true,"","side _this != WEST",12];
			};
	...
	if (_curatorCreate) then {
		...
		_curator addEventHandler ['CuratorObjectPlaced',{
			params ["_curator","_entity"];
			{
				...
				//if (INS_op_faction isEqualTo 20) then {[_x] call Trade_Biofoam_fnc};
			} forEach crew _entity;
			if ((_entity isKindOf "CAManBase") && {count units group _entity < 2}) then {
				if (!(side _entity isEqualTo INS_Op4_side) && {side _entity in [EAST,WEST]}) then {
		...
		_curator addEventHandler ['CuratorGroupPlaced',{
			params ["", "_group"];
			if (!(side leader _group isEqualTo INS_Op4_side) && {side leader _group in [EAST,WEST]}) then {
				_grp = createGroup INS_Op4_side;
				_units = units _group;
