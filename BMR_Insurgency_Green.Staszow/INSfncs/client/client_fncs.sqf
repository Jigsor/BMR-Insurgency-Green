...
JIG_UGVdrop_fnc = {
	...
	*/
	...
	ghst_ugvsupport = [(markerPos "ugv_spawn"),"I_UGV_01_rcws_F",3,30] execVM "scripts\ghst_ugvsupport.sqf";
	true
};
...
Op4_spawn_pos = {
	...
	_basePos = markerPos "Respawn_Guerrila";
	_players = [];
	...
	if (_posnotfound) then {
		if (INS_MHQ_exists && {!isNil "Opfor_MHQ"}) then {
			...
			if !(getMarkerColor "Opfor_MHQ" isEqualTo "") then {
				...
				"Respawn_West" setMarkerPos markerPos "Opfor_MHQ";
				_dir = random 359;
...
JIG_VA = {
	...
	if (playerside isEqualTo INDEPENDENT) then {_list = call BMRINS_fnc_BluforVA};
	if (playerside isEqualTo WEST) then {_list = call BMRINS_fnc_InsurgentVA};
