...
if (!_cache) then {
	...
	_eosActivated setTriggerActivation ["GUER","PRESENT",true];
...
if !(getmarkercolor _mkr isEqualTo "ColorBlack") then {
	...
	for "_counter" from 1 to _cGrps step 1 do {
		...
		if (_debug) then {player sidechat format ["Light Vehicle:%1 - r%2",_counter,_cGrps];0= [_mkr,_counter,"Light Veh",(getpos leader (_cGroup # 2))] call EOS_civ_debug};
	};
	...
	for "_counter" from 1 to _dGrps step 1 do {
		...
		if (_debug) then {player sidechat format ["Armoured:%1 - r%2",_counter,_dGrps];0= [_mkr,_counter,"Armour",(getpos leader (_dGroup # 2))] call EOS_civ_debug};
	};
	...
	for "_counter" from 1 to _eGrps step 1 do {
		...
		if (_debug) then {player sidechat format ["Static:%1",_counter];0= [_mkr,_counter,"Static",(getpos leader (_eGroup # 2))] call EOS_civ_debug};
	};
