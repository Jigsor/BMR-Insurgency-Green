...
// Relocate Op4 Base/"Respawn_West" marker position.
params ["_op4Player"];
...
_pnf = false;
_rwp = nil;
_bp = getMarkerPos "Respawn_Guerrila";
...
if (!isNil "_rwp") then {
	...
	if (count _sP > 0) exitWith {if (_mL) then {BTC_r_base_spawn setPos _sP}; "Respawn_West" setMarkerPos _sP;};
}else{
	_pnf = true;
};
