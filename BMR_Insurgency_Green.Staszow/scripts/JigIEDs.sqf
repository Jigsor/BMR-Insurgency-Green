...
{
	if ((_x distance2D (markerPos "Airfield") < 1000) ||
	{_x distance2D (markerPos "Respawn_West") < 750} ||
	{_x distance2D (markerPos "Respawn_East") < 200} ||
	{["bridge", getModelInfo _x select 0] call BIS_fnc_inString}) then {
		_roads = _roads - [_x];
	};
} forEach _roads;
...
for "_i" from 1 to _Active_IED_count step 1 do {
	...
	_IEDtrig setTriggerActivation ["GUER", "PRESENT", true];
...
if (_rmE) then {{west revealMine _x} count allIEDS};
if (_rmW) then {{independent revealMine _x} count allIEDS};
if (_rmI) then {{east revealMine _x} count allIEDS};
