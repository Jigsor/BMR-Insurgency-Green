...
if (INS_IEDs isEqualTo 1 && !IamHC) then {
	if !(INS_op_faction in [9,10,11,18,19]) then {0 spawn {call compile preprocessFileLineNumbers "EPD\Ied_Init.sqf"}};
};
...
switch (INS_op_faction) do {
	...
	// IFA3/FOW - Falscirmjagers (@CBA_A3;@CUP_Terrains_Core;@IFA3_AIO_LITE;@Faces of war)
		_requiredAddons = ["CBA_main","WW2_Assets_c_Characters_Americans_c_US_Rangers","fow_characters_c"];
		if ({isClass(configFile >> "cfgPatches" >> _x)} count _requiredAddons == count _requiredAddons) then {
			...
			activateAddons ["CBA_main","fow_boats_c","fow_characters_c","fow_weapons_c","fow_cars_c","fow_tanks_c","fow_tracked_c","fow_characters_items_c","fow_cannons_c","fow_statics_c","fow_mortars_c"];
			[WEST]execVM "eos\OpenMe.sqf";
		}else{INS_mod_missing = true;};
	};
	case 10: {
