...
0 spawn {
	...
	if (INS_VA_type in [0,3]) then {
		...
		MHQ_1 addAction[("<t color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{["Open",true] call BIS_fnc_arsenal}, nil, 6, true, true, "", "side _this != WEST", 12];
		MHQ_2 addAction[("<t color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{["Open",true] call BIS_fnc_arsenal}, nil, 6, true, true, "", "side _this != WEST", 12];
		MHQ_3 addAction[("<t color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{["Open",true] call BIS_fnc_arsenal}, nil, 6, true, true, "", "side _this != WEST", 12];
		...
		MHQ_1 addAction[("<t color='#F56618'>") + (localize "STR_BMR_load_VAprofile") + "</t>","=BTC=_revive\=BTC=_addAction.sqf",[[],JIG_load_VA_profile_MHQ1], 1, true, true, "", "side _this != WEST", 12];
		MHQ_2 addAction[("<t color='#F56618'>") + (localize "STR_BMR_load_VAprofile") + "</t>","=BTC=_revive\=BTC=_addAction.sqf",[[],JIG_load_VA_profile_MHQ2], 1, true, true, "", "side _this != WEST", 12];
		MHQ_3 addAction[("<t color='#F56618'>") + (localize "STR_BMR_load_VAprofile") + "</t>","=BTC=_revive\=BTC=_addAction.sqf",[[],JIG_load_VA_profile_MHQ3], 1, true, true, "", "side _this != WEST", 12];
	};
	if (INS_VA_type in [1,2]) then {
		...
		MHQ_1 addAction[("<t color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{[_this] call JIG_VA}, nil, 6, true, true, "", "side _this != WEST", 12];
		MHQ_2 addAction[("<t color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{[_this] call JIG_VA}, nil, 6, true, true, "", "side _this != WEST", 12];
		MHQ_3 addAction[("<t color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{[_this] call JIG_VA}, nil, 6, true, true, "", "side _this != WEST", 12];
	};

	// Blufor save respawn loadout
	if (INS_full_loadout isEqualTo 1) then {
		INS_Wep_box addAction[("<t size='1.5' shadow='2' color='#ff9207'>") + (localize "STR_BMR_save_loadout") + "</t>", {call INS_RespawnLoadout}, [], 1, false, true, "", "side _this != WEST", 15];
	};
	...
	if (INS_p_rev in [4,5]) then {
		...
		If (side player == west) then {player addEventHandler ["Killed", {Op4handle = [_this select 0] execVM "scripts\MoveOp4Base.sqf"}];};
	};
	...
	*/
	...
	If ((side player == INDEPENDENT && {INS_VA_type in [1,2]}) || (side player == west && {INS_VA_type in [2,3]})) then {
	...
	if (!(side player isEqualTo west) && {!(INS_PlayerPilot isEqualTo [])} && {!((typeOf player) in INS_PlayerPilot)}) then {
	...
	if (JIG_MHQ_enabled) then {
		0 spawn {
			...
			private _op4 = if (side player == west) then {TRUE}else{FALSE};
